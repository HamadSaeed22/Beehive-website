...
const BeeHiveMonitor = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const apiUrl = `https://script.google.com/macros/s/AKfycbyCgm1SpH2zBdPysIHdSf3Ublnpx5Zw3xKp6kXdenGpFs9si5Ou3z1vcbl5XR3nWkJxZA/exec?timeSlot=${timeSlot || "all"}`;
      ...
      const formattedData = data.map(entry => {
        ...
        const seconds = entryDate.getSeconds().toString().padStart(2, "0");
        const formattedTime = `${hours}:${minutes}:${seconds}`;

        return {
          ...entry,
          originalTime: entry.time, // Preserve the original timestamp for filtering
          time: formattedTime, // Replace the time field with the formatted time
          ...
          relayFan1: entry.relayFan1 === "on" ? "ON" : "OFF",
          relayFan2: entry.relayFan2 === "on" ? "ON" : "OFF",
        };
      });
  ...
  const handleTimeSlotChange = (slot) => {
    ...
    if (slot !== null && chartData.length > 0) {
      const now = new Date();
      const filteredData = chartData.filter((entry) => {
        const entryDate = new Date(entry.originalTime); // Use original timestamp for filtering
      ...
      // Reset to original data if "All Data" is selected
      fetchData();
    }
  };
      ...
      <header className="flex justify-between items-center p-6 shadow-md">
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            ...
            <option value="Beehive 2">Beehive 2</option> <option value="Beehive 2">Beehive 2</option>
          </select>
          <button
